...
matchMatrices haystack needle =
  ...
  let
    stencilSize     = (size needle)
    stencilCenter   = (0 :. 0)
    templateIndices = rangeSize Seq (0 :. 0) stencilSize

    stencil         = makeStencil stencilSize stencilCenter $ \get ->
      let
        -- multiply matrix a with matrix b and sum the results
        ab = Massiv.sum $ Massiv.map
          (\ix ->
            let s = get ix
                t = needle ! ix
            in  (* t) <$> s
...
matchImages sourceImage templateImage =
  let
      -- the template size of 15 was determined experimentally
    ...
    in
      go scaledSource scaledTemplate
        & (fmap . fmap) unscaleIx
