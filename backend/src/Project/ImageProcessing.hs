...
import Data.Massiv.Core (SizeException)
...
import qualified ImageProcessing.TemplateMatching as IPT
...
initModel glyphCollections =
  ImPModel
    { tmCollections = glyphCollections,
      tmProcess = Nothing,
...
update ctx@Ctx {..} msg model = case msg of
  ...
  StoreMatchedGlyphs matchedGlyphs ->
    let newCollections =
          GlyphCollection.appendMatchedGlyphs
            matchedGlyphs
            (tmCollections model)
...
runTemplateMatching Ctx {..} input prevCollections = do
  ...
  where
    hasMatchedBefore collection (source, template) =
      let prevMatched =
            collection & List.concatMap gcMatches
              & List.map
